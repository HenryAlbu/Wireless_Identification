<?php
$conn = mysqli_connect('206.123.181.206', 'pi2', 'piDBpassword', 'tracking_system');
...
foreach($station_dump as $scan) {	
	...
	if(preg_match('/^(.+)[[:space:]]+(.+)[[:space:]]+(.+)$/', $scan, $station_dump_matches) == 1) {
		...
		$sql = "UPDATE tracking SET pi2 = '$signal' WHERE mac = '$mac'";			
		$conn->query($sql);    

		$query = "SELECT * from tracking where mac = '$mac'";
		$result = mysqli_query($conn,$query);	

		if(mysqli_num_rows($result) > 0)
		{
			
			$sql = "UPDATE tracking SET tracking.status = 1 WHERE tracking.mac = '$mac'";			
			$conn->query($sql);
